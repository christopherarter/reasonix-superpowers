...
**Invocation** measures which skill fires. `bench/bench.mjs` runs a structural gate over every `SKILL.md`, then sends realistic prompts through `reasonix run` and reads back the chosen skill. The 12-case corpus (one prompt per skill plus two negatives) passes 12/12: the expected skill fires first on all 10 positives, and neither negative triggers anything.
