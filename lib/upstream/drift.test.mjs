...
test("advanced commit but no watched files changed → no drift", () => {
  const r = computeDrift(manifest, { obra: "obra111", reasonix: "rx000" },
    { obra: ["README.md", "docs/something.md"], reasonix: null });
  assert.equal(r.obra.drifted, true);
  assert.equal(r.obra.items.length, 0);
  assert.equal(hasDrift(r), false);
});

test("degraded compare → still drift", () => {
